...
1. **Residual demand inelasticity.** Insureds now draw heterogeneous reservation prices from `LogNormal(ln(0.25), 0.40)`, producing a downward-sloping demand curve. At hard-market rates (~14–21%), 7–33% of insureds price out, reducing the pool available to new entrants. However, insureds that do buy still purchase at full `sum_insured` with no attachment — there is no quantity adjustment (limit reduction, deductible increase, self-insurance). New capital therefore still competes for the same per-policy premium volume, and rate collapse remains faster than in the real market.
