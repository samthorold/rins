...
- Rate-on-line oscillates between ~11.5–16.0% RoL. `ApTp` ranges from 0.90 (soft floor) to 1.45 (year 124, the most severe event in the 200-year run: 6 cat events, 225% LossR). Both directions of rate movement are present, and the hard-market spike in year 124 sustains elevated rates for 3–4 years before softening.
- 11 years exceed 100% LossR over 200 years, all cat-driven. Capital entry fires correctly: 7 entrants cluster after severe years (yrs 45, 92, 124–126, 134, 138). Market grows from 8 insurers to 15 by year 138 and stabilises there.
- TotalCap grows monotonically: 1.26B (yr6) → 8.01B (yr205). No insolvencies in 200 years despite a 225% LossR year.

**Hard market collapse — remaining structural gaps:** individual pricing (Phase 1) and demand elasticity (Phase 4) are active. Two gaps remain:

1. **No soft-market supply contraction.** With `line_size = 1.0` fixed, the only way an insurer reduces exposure is to decline entirely. Binary decline produces `SubmissionDropped` which is correctly informative but does not allow the insurer to signal partial willingness. In the real market, syndicates shade their line down as rates soften; capacity contraction is continuous and insurer-specific, not a threshold switch. Variable line sizes (roadmap Phase 5) close this gap: `pricing_line` falls toward zero as `own_ap_tp_factor` approaches the floor, producing a smooth soft-market capacity reduction without any agent exiting.

2. **Unbounded capital accumulation erodes crisis severity.** TotalCap grows 6.3× over 200 years while max GUL stays fixed at ~1.25B. By year 200 the market can absorb a 225% LossR event with a 9.5% capital dip. Hard-market pricing signals fire correctly but there is no capacity constraint behind them — the market is structurally overcapitalised relative to its exposure pool. Capital distributions (roadmap Phase 6) drain surplus annually, keeping capital near the exposure-appropriate level and maintaining the vulnerability regime in which crises produce genuine capacity crunches rather than cosmetic rate movements.
