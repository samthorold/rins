...
| 17 | `InsurerExited { insurer_id }` | `Insurer::on_year_end` | Logged directly by `handle_year_end` (not dispatched); `Broker::remove_insurer` removes insurer from round-robin; counted in `Exits#` column | `YearEnd` day | §7 Capital & Solvency — exit criterion: own 3yr avg CR > `runoff_cr_threshold` (1.05) AND capital > `capital_exit_floor` × initial_capital (0.90); in-force policies run to natural expiry |
| 18 | `InsurerReEntered { insurer_id }` | `Simulation::handle_year_end` | Logged directly (not dispatched); insurer's `in_runoff` flag cleared; `Broker::add_insurer` restores round-robin participation | `YearEnd` day | §7 Capital & Solvency — re-entry criterion: `market_ap_tp_factor > 1.10`; symmetric to new entrant threshold |
