...
Invariant: every QuoteRequested for (submission_id, syndicate_id) must have
...
requested = {}    # (submission_id, syndicate_id) -> day
responses = defaultdict(list)  # (submission_id, syndicate_id) -> [event_type, ...]
...
for e in events:
    ...
    if k == "QuoteRequested":
        key = (v["submission_id"], v["syndicate_id"])
        requested[key] = day
...
for key, req_day in sorted(requested.items()):
    if key not in responses:
        sub_id, syn_id = key
        failures.append(
            f"  FAIL submission_id={sub_id} syndicate_id={syn_id}: "
